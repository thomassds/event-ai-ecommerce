...
import { Event, Lot } from "@/interfaces";
import { GenericAccordion } from "../accordions";
import { useAppAuth, useAppCheckout } from "@/hooks";

const ITEMS_PER_PAGE_DEFAULT = 3;

const VariationTaxInfo = ({ lot }: { lot: Lot }) => {
...
export const TicketSelectorClient = ({ event }: TicketSelectorClientProps) => {
  const { isLoggedIn } = useAppAuth();
      ...
      return [
        {
          ...
          content: (
            <div className="space-y-4">
              <div>Nenhum setor disponível para este evento.</div>
            </div>
          ),
        },
      ];
      ...
      content: (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-3">
            <input
              type="text"
              value={groupSearch[sector.id] || ""}
              onChange={(e) => handleSearchChange(sector.id, e.target.value)}
              placeholder="Buscar por variação ou lote"
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              aria-label={`Buscar ingressos do grupo ${sector.name}`}
            />
          </div>

          {(() => {
            const term = (groupSearch[sector.id] || "").toLowerCase();
            const filtered =
              sector.tickets?.filter((v) => {
                // Filtrar apenas por termo de busca - MANTER ingressos esgotados visíveis
                const matchesSearch = [v.name, String(v.name || "")].some((f) =>
            ...
            const page = getPageFor(sector.id);
            ...
            return (
              <>
                {pageItems.map((ticket, varIndex) => {
                  const lot = ticket.lots?.length ? ticket.lots[0] : null;
                  if (!lot) return null;

                  const currentQuantity =
                    lotsSelected[lot?.id]?.quantitySelected || 0;

                  const lotIdString = String(lot.id);

                  // Verificar se o ingresso está esgotado
                  const isOutOfStock = !lot.isActive;
                        ...
                        {!isOutOfStock ? (
                          <div className="flex flex-col items-center gap-2 border border-gray-200 rounded-lg p-2">
                            <button
                              ...
                              aria-label={`Adicionar ingresso ${ticket.name}`}
                              title={
                                !isUserAuthenticated
                                  ? "Faça login para selecionar ingressos"
                            ...
                            <button
                              onClick={() => removeLotSelected(lot.id)}
                              disabled={
                                !isUserAuthenticated ||
                                currentQuantity <= 0 ||
                                loadingTaxes[lotIdString]
                              ...
                              className="w-8 h-8 rounded-md bg-purple-100 hover:bg-purple-200 disabled:bg-gray-100 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
                              aria-label={`Remover ingresso ${ticket.name}`}
                              title={
                                !isUserAuthenticated
                                  ? "Faça login para selecionar ingressos"
                                  : loadingTaxes[lotIdString]
                      ...
                      {isOutOfStock && (
                        <div className="mb-4 p-3 bg-yellow-50 border-l-4 border-yellow-500 rounded-r-lg">
                          <div className="flex items-start">
                            ...
                            <div className="ml-3">
                              {ticket.name.toLowerCase().includes("meia") ? (
                                <>
                ...
                {needsPagination && (
                  <div className="flex items-center justify-between pt-2">
                    <button
                      ...
                      onClick={() =>
                        setPageFor(sector.id, Math.max(1, page - 1))
                      }
                      disabled={page === 1}
